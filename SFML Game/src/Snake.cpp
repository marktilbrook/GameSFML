...
Snake::Snake(int a_blockSize)
{
	m_size = a_blockSize;
	m_bodyRect.setSize(sf::Vector2f(m_size, m_size - 1));
	Reset();
}
...
void Snake::SetDirection(Direction a_dir)
{
	m_dir = a_dir;
}
...
void Snake::Reset()
{
	...
	SetDirection(Direction::None); //Starts off still
	m_speed = 15;
	m_lives = 3;
	m_score = 0;
	m_lost = false;
...
void Snake::Render(sf::RenderWindow & a_window)
{
	if (m_snakeBody.empty())
	{
		return;
